...
def init_dir(path, station):
    cloud_dir = path / station / "cloud_{}0000{}.csv".format(station[0], station[1])
    sunny_dir = path / station / "solar_{}0000{}.csv".format(station[0], station[1])
    weather_dir = path / station / "weather_forecast_{}0000{}.csv".format(station[0], station[1])
    e_generator_dir = path /station /  "realne_report_solar_30_{}0000{}_{}_{}.xlsx".format(station[0], station[1], start_date[0], end_date[1])
    e_demand_dir = list()
    if station[0] == "v":
        e_demand_dir.append(path /station /  "realne_report_surplus30p_{}0000{}_{}_{}.xlsx".format(station[0], station[1], start_date[0], end_date[0]))
        e_demand_dir.append(path /station /  "realne_report_surplus30p_{}0000{}_{}_{}.xlsx".format(station[0], station[1], start_date[1], end_date[1]))
    else:
        e_demand_dir.append(path /station /  "realne_report_surplus30p_{}0000{}_{}_{}.xlsx".format(station[0], station[1], start_date[0], end_date[1]))
...
def create_dataset(path):
    ...
    for station in stations:
        ...
        # Save station data
        station_data.to_csv(output_dir + "{}_dataset.csv".format(station))
        
        full_dataset = pd.concat([full_dataset, station_data])
        
    full_dataset.to_csv(output_dir + "full_dataset.csv")
