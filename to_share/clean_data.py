...
def create_csv(cloud_dir, solar_dir, weather_dir):
    ...
    cloud_data = pd.read_csv(cloud_dir).drop(columns = "execution_date")
    solar_data = pd.read_csv(solar_dir).drop(columns = "execution_date")
    weather_data = pd.read_csv(weather_dir).drop(columns = "execution_date")

    csv_data = pd.merge(weather_data, solar_data, how = 'outer', on = ["target_date"])
